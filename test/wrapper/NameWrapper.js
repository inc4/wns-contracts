...
const GRACE_PERIOD = 30 * ONE_DAY_IN_SEC
const WBT_TLD = 'wbt'
...
describe('Name Wrapper', () => {
  ...
  describe('wrap()', () => {
    ...
    it('Does not allow anyone else to wrap a name even if the owner has authorized the wrapper with the ENS registry.', async () => {
      const labelHash = labelhash('abc')
    ...
    it('Does not allow wrapping .wbt 2LDs.', async () => {
      const label = 'wrapped'
      const labelHash = labelhash(label)
      await BaseRegistrar.register(labelHash, account, 1 * ONE_DAY_IN_SEC)
      await EnsRegistry.setApprovalForAll(NameWrapper.address, true)
      await expect(
        NameWrapper.wrap(encodeName('wrapped.wbt'), account2, EMPTY_ADDRESS),
    ...
    it('Rewrapping a previously wrapped unexpired name retains PCC', async () => {
      const label = 'test'
      const labelHash = labelhash(label)
      const wrappedTokenId = namehash(label + '.wbt')
      const subLabel = 'sub'
      const subLabelHash = labelhash(subLabel)
      const subWrappedTokenId = namehash(`${subLabel}.${label}.wbt`)
  ...
  describe('unwrap()', () => {
    ...
    it('Will not allow previous owner to unwrap name when name expires', async () => {
      ...
      await evm.advanceTime(GRACE_PERIOD + 1 * ONE_DAY_IN_SEC + 1)
      await evm.mine()
      ...
      await expect(
        ...
        `Unauthorised("${namehash('sub.unwrapped.wbt')}", "${account}")`,
      )
    })
    ...
    it('Will not unwrap .wbt 2LDs.', async () => {
      ...
      await expect(
        NameWrapper.unwrap(namehash(WBT_TLD), labelhash('unwrapped'), account),
      ).to.be.revertedWith('IncompatibleParent()')
    })
    ...
    it('Will not allow to unwrap with PCC/CU burned if expired', async () => {
      ...
      expect(await EnsRegistry.owner(namehash('sub.awesome.wbt'))).to.equal(
        NameWrapper.address,
      )

      await expect(
        NameWrapper.unwrap(namehash('awesome.wbt'), labelhash('sub'), account),
      ).to.be.revertedWith(
        `Unauthorised("${namehash('sub.awesome.wbt')}", "${account}")`,
      )
    })
    ...
    it('Will allow to unwrap with PCC/CU burned if expired and then extended without PCC/CU', async () => {
      const label = 'awesome'
      const labelHash = labelhash(label)
      await BaseRegistrar.register(labelHash, account, 1 * ONE_DAY_IN_SEC * 7)
      await EnsRegistry.setSubnodeOwner(
        namehash('awesome.wbt'),
        labelhash('sub'),
        account,
      ...
      await evm.advanceTime(2 * ONE_DAY_IN_SEC)
      await evm.mine()
    ...
    it('Will not allow to unwrap a name with the CANNOT_UNWRAP fuse burned if not expired', async () => {
      ...
      await expect(
        NameWrapper.unwrap(namehash('abc.wbt'), labelhash('sub'), account),
      ).to.be.revertedWith(`OperationProhibited("${namehash('sub.abc.wbt')}")`)
    })
  ...
  describe('wrapETH2LD()', () => {
    ...
    const encodedName = encodeName(`${label}.${WBT_TLD}`)
    ...
    it('Cannot wrap a name if the owner has not authorized the wrapper with the .wbt registrar.', async () => {
      await BaseRegistrar.register(labelHash, account, 1 * ONE_DAY_IN_SEC)
    ...
    it('Can re-wrap a name that was wrapped has already expired on the .wbt registrar', async () => {
      await BaseRegistrar.register(labelHash, account, 1 * ONE_DAY_IN_SEC)
      await BaseRegistrar.setApprovalForAll(NameWrapper.address, true)
      await NameWrapper.wrapETH2LD(
        label,
      ...
      await increaseTime(ONE_DAY_IN_SEC * GRACE_PERIOD + ONE_DAY_IN_SEC + 1)
      await mine()

      expect(await BaseRegistrar.available(labelHash)).to.equal(true)
      await BaseRegistrar2.register(labelHash, account2, 1 * ONE_DAY_IN_SEC)
    ...
    it('Can re-wrap a name that was wrapped has already expired even if CANNOT_TRANSFER was burned', async () => {
      ...
      await expect(tx)
        .to.emit(NameWrapper, 'NameUnwrapped')
        .withArgs(namehash('wrapped2.wbt'), EMPTY_ADDRESS)
      await expect(tx)
        .to.emit(NameWrapper, 'TransferSingle')
        .withArgs(account2, account, EMPTY_ADDRESS, nameHash, 1)
      await expect(tx)
        .to.emit(NameWrapper, 'NameWrapped')
        .withArgs(
          namehash('wrapped2.wbt'),
          encodeName('wrapped2.wbt'),
          account2,
          PARENT_CANNOT_CONTROL | IS_DOT_ETH,
          expectedExpiry.add(GRACE_PERIOD),
        )
    ...
    it('correctly reports fuses for a name that has expired and been rewrapped more permissively', async () => {
      ...
      ;[, fuses] = await NameWrapper.getData(namehash('sub.wrapped2.wbt'))
      ...
      ;[, fuses, expiry] = await NameWrapper.getData(namehash('wrapped2.wbt'))
    ...
    it('Allows an account approved by the owner on the .wbt registrar to wrap a name.', async () => {
      await BaseRegistrar.register(labelHash, account, 1 * ONE_DAY_IN_SEC)
      await BaseRegistrar.setApprovalForAll(NameWrapper.address, true)
    ...
    it('Allows fuse to be burned if CANNOT_UNWRAP has been burned, but resets to 0 if expired', async () => {
      ...
      await increaseTime(ONE_DAY_IN_SEC + 1 + GRACE_PERIOD)
      await mine()
      const [, fuses] = await NameWrapper.getData(nameHash)
      expect(fuses).to.equal(0)
    })
  ...
  describe('unwrapETH2LD()', () => {
    const label = 'unwrapped'
    const labelHash = labelhash(label)
    const nameHash = namehash(label + '.wbt')
    it('Allows the owner to unwrap a name.', async () => {
      ...
      expect(await NameWrapper.ownerOf(namehash('unwrapped.wbt'))).to.equal(
        account,
      )
      await NameWrapper.unwrapETH2LD(labelHash, account, account)
      // transfers the controller on the .wbt registrar to the target address.
      expect(await EnsRegistry.owner(namehash('unwrapped.wbt'))).to.equal(
        account,
      )
      //Transfers the registrant on the .wbt registrar to the target address
    ...
    it('Does not allows the previous owner to unwrap when the name has expired.', async () => {
      ...
      await increaseTime(ONE_DAY_IN_SEC + 1)
      await mine()
      await expect(
    ...
    it('emits Unwrap event', async () => {
      ...
      await expect(tx)
        .to.emit(NameWrapper, 'NameUnwrapped')
        .withArgs(namehash('unwrapped.wbt'), account)
    })
    ...
    it('Does not allow a name to be unwrapped if CANNOT_UNWRAP fuse has been burned', async () => {
      ...
      await expect(
        ...
        'OperationProhibited("0x0dbbcf9e9ed157133da6e184e4bfa0591303e0ae864c22f37873df19d94423b6")',
      )
    })
    it('Unwrapping a previously wrapped unexpired name retains PCC and expiry', async () => {
      // register and wrap a name with PCC
  ...
  describe('ownerOf()', () => {
    it('Returns the owner', async () => {
      ...
      await BaseRegistrar.register(tokenId, account, 1 * ONE_DAY_IN_SEC)
    ...
    it('Returns 0 when owner is expired', async () => {
      ...
      await evm.advanceTime(1 * ONE_DAY_IN_SEC + GRACE_PERIOD + 1)
      await evm.mine()
  ...
  describe('approve()', () => {
    ...
    const subWrappedTokenId = namehash('sub.' + label + '.wbt')
    let parentExpiry
    let result
    before(async () => {
      result = await ethers.provider.send('evm_snapshot')
    ...
    it('Does not allow approved accounts to extend expiry when expired', async () => {
      ...
      await expect(
        ...
        `OperationProhibited("${namehash('sub.subdomain.wbt')}")`,
      )
    })
    ...
    it('Approved address cannot unwrap .wbt', async () => {
      await NameWrapper.approve(account2, wrappedTokenId)
      await expect(
    ...
    it('Approved address cannot unwrap non .wbt', async () => {
      await NameWrapper.setSubnodeOwner(
        wrappedTokenId,
        'sub',
        account,
      ...
      await NameWrapper.approve(account2, namehash('sub.subdomain.wbt'))
      await expect(
        NameWrapper2.unwrap(wrappedTokenId, labelhash('sub'), account2),
      ).to.be.revertedWith(
        `Unauthorised("${namehash('sub.subdomain.wbt')}", "${account2}")`,
      )
    })
    ...
    it('Approval is cleared on unwrap()', async () => {
      ...
      await NameWrapper.approve(account2, namehash('sub.subdomain.wbt'))
      expect(
        await NameWrapper.getApproved(namehash('sub.subdomain.wbt')),
      ).to.equal(account2)
    ...
    it('Approval is cleared on re-registration and wrap of expired name', async () => {
      ...
      await NameWrapper.setFuses(wrappedTokenId, CANNOT_UNWRAP | CANNOT_APPROVE)
      expect(await NameWrapper.getApproved(wrappedTokenId)).to.equal(account2)
      await evm.advanceTime(2 * ONE_DAY_IN_SEC + GRACE_PERIOD)
      await evm.mine()
  ...
  describe('getApproved()', () => {
    ...
    it('Returns returns zero address when ownerOf() is zero', async () => {
      expect(await NameWrapper.ownerOf(namehash('unminted.wbt'))).to.equal(
        EMPTY_ADDRESS,
      )
      expect(await NameWrapper.getApproved(namehash('unminted.wbt'))).to.equal(
        EMPTY_ADDRESS,
      )
    })
  ...
  describe('upgrade()', () => {
    describe('.wbt', () => {
      const encodedName = encodeName('wrapped2.wbt')
      const label = 'wrapped2'
      const labelHash = labelhash(label)
      const nameHash = namehash(label + '.wbt')

      it('Upgrades a .wbt name if sender is owner', async () => {
        await BaseRegistrar.register(labelHash, account, 1 * ONE_DAY_IN_SEC)
        const expectedExpiry = await BaseRegistrar.nameExpires(labelHash)
        await BaseRegistrar.setApprovalForAll(NameWrapper.address, true)
      ...
      it('Upgrades a .wbt name if sender is authorised by the owner', async () => {
        await BaseRegistrar.register(labelHash, account, 1 * ONE_DAY_IN_SEC)
        const expectedExpiry = await BaseRegistrar.nameExpires(labelHash)
        await BaseRegistrar.setApprovalForAll(NameWrapper.address, true)
        await NameWrapper.setApprovalForAll(account2, true)
    ...
    describe('other', () => {
      ...
      const name = label + '.' + parentLabel + '.wbt'
      const parentLabelHash = labelhash(parentLabel)
      const parentHash = namehash(parentLabel + '.wbt')
  ...
  describe('setFuses()', () => {
    ...
    const wrappedTokenId = namehash('fuses.wbt')
    ...
    it('cannot burn fuses as the previous owner of a .wbt when the name has expired', async () => {
      await BaseRegistrar.register(
        labelhash('abc'),
        account,
        1 * ONE_DAY_IN_SEC,
      ...
      await expect(
        NameWrapper.setFuses(namehash('abc.wbt'), CANNOT_UNWRAP),
      ).to.be.revertedWith(
        `Unauthorised("${namehash('abc.wbt')}", "${account}")`,
      )
    })

    it('cannot burn fuses as a previous owner of a non .wbt when the name has expired', async () => {
      await BaseRegistrar.register(
        labelhash('abc'),
        account,
        1 * ONE_DAY_IN_SEC,
      ...
      await expect(
        NameWrapper.setFuses(namehash('sub.abc.wbt'), CANNOT_UNWRAP),
      ).to.be.revertedWith(
        `Unauthorised("${namehash('sub.abc.wbt')}", "${account}")`,
      )
    })
    ...
    it('Will not allow burning fuses if PARENT_CANNOT_CONTROL has not been burned', async () => {
      ...
      // 0xdd79adbb8f7236fe2b6e13df741f4482f7274cefe5af142f82cd40c25d8541bf = namehash('sub.abc.wbt')
      ...
      await expect(
        ...
        `OperationProhibited("0xdd79adbb8f7236fe2b6e13df741f4482f7274cefe5af142f82cd40c25d8541bf")`,
      )
    })
    ...
    it('Will not allow burning fuses of subdomains if CANNOT_UNWRAP has not been burned', async () => {
      ...
      await expect(
        NameWrapper.setFuses(namehash('sub.abc.wbt'), CANNOT_TRANSFER),
      ).to.be.revertedWith(
        `OperationProhibited("0xdd79adbb8f7236fe2b6e13df741f4482f7274cefe5af142f82cd40c25d8541bf")`,
      )
    })

    it('Will not allow burning fuses of .wbt names unless CANNOT_UNWRAP is also burned.', async () => {
      await BaseRegistrar.register(tokenId, account, 1 * ONE_DAY_IN_SEC)
    ...
    it('can set fuses and burn transfer', async () => {
      ...
      const wrappedTokenId = namehash('fuses3.wbt')
    ...
    it('can set fuses and burn canCreateSubdomains', async () => {
      ...
      expect(
        await EnsRegistry.owner(namehash('creatable.fuses2.wbt')),
      ).to.equal(NameWrapper.address)

      expect(
        await NameWrapper.ownerOf(namehash('creatable.fuses2.wbt')),
      ).to.equal(account)
      ...
      await expect(
        ...
        `OperationProhibited("${namehash('uncreatable.fuses2.wbt')}")`,
      )
    })
  })
  ...
  describe('extendExpiry()', () => {
    ...
    it('Allows parent owner to set expiry without CAN_EXTEND_EXPIRY burned', async () => {
      ...
      ;[, fuses, expiry] = await NameWrapper.getData(namehash('sub.fuses.wbt'))
    ...
    it('Does not allow owner of .wbt 2LD to set expiry', async () => {
      await registerSetupAndWrapName('fuses', account, CANNOT_UNWRAP)

      let [, fuses, expiry] = await NameWrapper.getData(namehash('fuses.wbt'))

      expect(fuses).to.equal(IS_DOT_ETH | PARENT_CANNOT_CONTROL | CANNOT_UNWRAP)

      await expect(
        NameWrapper.extendExpiry(namehash(WBT_TLD), tokenId, expiry),
      ).to.be.revertedWith(`OperationProhibited("${wrappedTokenId}")`)
    })
    ...
    it('Does not allow .wbt 2LD owner to set expiry on child if the .wbt 2LD is expired but grace period has not ended', async () => {
      await registerSetupAndWrapName('fuses', account, CANNOT_UNWRAP)
      const parentExpiry = await BaseRegistrar.nameExpires(tokenId)
      await NameWrapper.setSubnodeOwner(
        wrappedTokenId,
    ...
    it('Allows child owner to set expiry if parent .wbt 2LD is expired but grace period has not ended', async () => {
      await registerSetupAndWrapName('fuses', account, CANNOT_UNWRAP)
      const parentExpiry = await BaseRegistrar.nameExpires(tokenId)
      await NameWrapper.setSubnodeOwner(
        wrappedTokenId,
    ...
    it('Does not allow child owner to set expiry if Emancipated child name has expired', async () => {
      ...
      await NameWrapper.setSubnodeOwner(
        ...
        parentExpiry - ONE_DAY_IN_SEC + 3600,
      )
      ...
      expect(expiry).to.equal(parentExpiry - ONE_DAY_IN_SEC + 3600)
    ...
    it('Does not allow extendExpiry() to be called on unregistered names (expired w/ PCC burnt)', async () => {
      ...
      await NameWrapper.setSubnodeOwner(
        ...
        account,
        PARENT_CANNOT_CONTROL,
        parentExpiry - 5 * ONE_DAY_IN_SEC,
      )

      // Advance time so the subdomain expires, but not the parent
      await evm.advanceTime(5 * ONE_DAY_IN_SEC + 1)
      await evm.mine()
    ...
    it('Does not allow extendExpiry() to be called on unwrapped names', async () => {
      ...
      let registryOwner = await EnsRegistry.owner(namehash('sub.fuses.wbt'))
    ...
    it('Emits Expiry Extended event', async () => {
      ...
      await expect(tx)
        ...
        .withArgs(namehash(`sub.${label}.wbt`), parentExpiry.add(GRACE_PERIOD))
    })
  })
  ...
  describe('setChildFuses()', () => {
    ...
    it('Does not allow parent owners to burn parent controlled fuses after burning PCC', async () => {
      ...
      await expect(
        ...
        `OperationProhibited("${namehash('sub.fuses.wbt')}")`,
      )
    })
    ...
    it('Does not allow non-parent owners to set child fuses', async () => {
      const subWrappedTokenId = namehash('sub.fuses.wbt')
      await registerSetupAndWrapName('fuses', account, CANNOT_UNWRAP)
      await NameWrapper.setSubnodeOwner(wrappedTokenId, 'sub', account, 0, 0)
    ...
    it('should not allow .wbt to call setChildFuses()', async () => {
      await registerSetupAndWrapName('fuses', account, 0)
      ...
      ).to.be.revertedWith(`Unauthorised("${namehash(WBT_TLD)}", "${account}")`)
    })
    ...
    it('Fuses and owner are set to 0 if expired and fuses cannot be burnt after expiry using setChildFuses()', async () => {
      ...
      await expect(
        NameWrapper.setChildFuses(
          ...
          labelhash('sub'),
          PARENT_CANNOT_CONTROL | CANNOT_UNWRAP,
          block.timestamp + 1 * ONE_DAY_IN_SEC,
        ),
      ).to.be.revertedWith(`NameIsNotWrapped()`)
    })
  })
  ...
  describe('setSubnodeOwner()', async () => {
    ...
    it('Can be called by the owner of a name and sets this contract as owner on the ENS registry.', async () => {
      ...
      expect(await EnsRegistry.owner(namehash(`sub.${label}.wbt`))).to.equal(
        NameWrapper.address,
      )

      expect(await NameWrapper.ownerOf(namehash(`sub.${label}.wbt`))).to.equal(
        account,
      )
    })
    it('Can be called by an account authorised by the owner.', async () => {
    ...
    it('Fuses cannot be burned if the name does not have PARENT_CANNOT_CONTROL burned', async () => {
      ...
      await expect(
        ...
        `OperationProhibited("${namehash(`${label2}.${label}.wbt`)}")`,
      )
    })
    ...
    it('Does not allow IS_DOT_ETH to be burned', async () => {
      ...
      await expect(
        ...
        `OperationProhibited("${namehash(`sub.${label}.wbt`)}")`,
      )
    })
    ...
    it('Unwrapping within an external contract does not create any state inconsistencies', async () => {
      ...
      // set self as sub.test.wbt owner
      await NameWrapper.setSubnodeOwner(
        namehash('test.wbt'),
        'sub',
        account,
    ...
    it('Rewrapping a name that had PCC burned, but has now expired is possible and resets fuses', async () => {
      ...
      expect(owner).to.equal(EMPTY_ADDRESS)
      expect(expiry).to.equal(parentExpiry - ONE_DAY_IN_SEC / 2)
      expect(fuses).to.equal(PARENT_CANNOT_CONTROL)

      // Advance time so the subdomain expires, but not the parent
      await evm.advanceTime(ONE_DAY_IN_SEC / 2 + 1)
      await evm.mine()

      const [, fusesAfter, expiryAfter] = await NameWrapper.getData(
        subWrappedTokenId,
      )
      expect(expiryAfter).to.equal(parentExpiry - ONE_DAY_IN_SEC / 2)
    ...
    it('Expired subnames should still be protected by CANNOT_CREATE_SUBDOMAIN on the parent', async () => {
      ...
      const subWrappedTokenId = namehash(`${subLabel}.${label}.wbt`)
      const subWrappedTokenId2 = namehash(`${subLabel2}.${label}.wbt`)
      await registerSetupAndWrapName(label, account, CANNOT_UNWRAP)
      const parentExpiry = await BaseRegistrar.nameExpires(labelHash)
      // Confirm that the name is wrapped
      expect(await NameWrapper.ownerOf(wrappedTokenId)).to.equal(account)
  ...
  describe('setSubnodeRecord()', async () => {
    ...
    it('Fuses will remain 0 if expired', async () => {
      ...
      const [, fuses] = await NameWrapper.getData(namehash(`sub.${label}.wbt`))
      expect(fuses).to.equal(0)
    })
    ...
    it('Sets the appropriate values on the ENS registry', async () => {
      ...
      const node = namehash(`sub.${label}.wbt`)
  ...
  describe('setRecord()', () => {
    ...
    it('Setting the owner to 0 reverts if CANNOT_UNWRAP is burned', async () => {
      ...
      const wrappedTokenId2 = namehash('setrecord2.wbt')
      const subWrappedTokenId = namehash('sub.setrecord2.wbt')
      await NameWrapper.setSubnodeOwner(
        wrappedTokenId2,
        'sub',
        account,
    ...
    it('Setting the owner to 0 on a .wbt reverts', async () => {
      await registerSetupAndWrapName('setrecord2', account, 0)
      const wrappedTokenId2 = namehash('setrecord2.wbt')
  ...
  describe('onERC721Received', () => {
    ...
    it('Allows burning other fuses if CAN_UNWRAP has been burnt, but resets fuses if expired', async () => {
      ...
      await evm.advanceTime(GRACE_PERIOD + 1 * ONE_DAY_IN_SEC)
      await evm.mine()
  ...
  describe('getData', () => {
    ...
    const subLabel = 'sub'
    const subLabelHash = labelhash(subLabel)
    const subNameHash = namehash(`${subLabel}.${label}.wbt`)
    const subSubLabel = 'subsub'
    const subSubLabelhash = labelhash(subSubLabel)
    const subSubNameHash = namehash(`${subSubLabel}.${subLabel}.${label}.wbt`)
  ...
  describe('registerAndWrapETH2LD()', () => {
    ...
    it('Does not allows fuse to be burned if CANNOT_UNWRAP has not been burned.', async () => {
      ...
      ).to.be.revertedWith(`OperationProhibited("${namehash(label + '.wbt')}")`)
    })
  ...
  describe('renew()', () => {
    ...
    it('Renewing name less than required to unexpire it still has original owner/fuses', async () => {
      ...
      await evm.advanceTime(ONE_DAY_IN_SEC * 2)
      await mine()
      ...
      await NameWrapper.renew(labelHash, 1 * ONE_DAY_IN_SEC)
  ...
  describe('isWrapped(bytes32 node)', () => {
    ...
    it('identifies a wrapped .wbt name', async () => {
      expect(await NameWrapper['isWrapped(bytes32)'](wrappedTokenId)).to.equal(
        true,
      )
    })

    it('identifies an expired .wbt name as unwrapped', async () => {
      const label = 'expired'
      const wrappedTokenId = namehash(label + '.wbt')
      await registerSetupAndWrapName(
        label,
      ...
      await evm.advanceTime(1 * ONE_DAY_IN_SEC + 1)
      await evm.mine()
      expect(await NameWrapper['isWrapped(bytes32)'](wrappedTokenId)).to.equal(
        false,
      )
    ...
    it('identifies an wbt name registered on old controller as unwrapped', async () => {
      const label = 'oldcontroller'
      const labelHash = labelhash(label)
      const tokenId = namehash('oldcontroller.wbt')
      await BaseRegistrar.register(labelHash, account, 1 * ONE_DAY_IN_SEC)
    ...
    it('identifies an unregistered .wbt name as unwrapped', async () => {
      const tokenId = namehash('abcdefghijklmnop.wbt')
      expect(await NameWrapper['isWrapped(bytes32)'](tokenId)).to.equal(false)
    })
    ...
    it('identifies a wrapped subname', async () => {
      await NameWrapper.setSubnodeOwner(wrappedTokenId, 'sub', account, 0, 0)
      const subTokenId = namehash('sub.something.wbt')
    ...
    it('identifies an expired wrapped subname with PCC burnt as unwrapped', async () => {
      ...
      await evm.advanceTime(ONE_DAY_IN_SEC + GRACE_PERIOD + 101)
      await evm.mine()
  ...
  describe('NameWrapper.names preimage dictionary', () => {
    it('Does not allow manipulating the preimage db by manually setting owner as NameWrapper', async () => {
      ...
      const labelHash = labelhash(label)
      const wrappedTokenId = namehash(label + '.wbt')
      await BaseRegistrar.register(labelHash, hacker, 1 * ONE_DAY_IN_SEC)
      ...
      const sub1Domain = sub1Label + '.' + label + '.wbt' // sub1.base.wbt
      ...
      const sub2Domain = sub2Label + '.' + sub1Domain // sub2.sub1.base.wbt
      ...
      const sub3Label = WBT_TLD
      const sub3LabelHash = labelhash(sub3Label)
      const sub3Domain = sub3Label + '.' + sub2Domain // wbt.sub2.sub1.base.wbt
  ...
  describe('Grace period tests', () => {
    ...
    const wrappedTokenId = namehash(label + '.wbt')
    const subLabel = 'sub'
    const subTokenId = namehash(subLabel + '.' + label + '.wbt')
    let parentExpiry
    before(async () => {
      await registerSetupAndWrapName(label, account, CANNOT_UNWRAP)
      ;[, , parentExpiry] = await NameWrapper.getData(wrappedTokenId)
      ...
      // move .wbt name to expired and be within grace period
      await evm.advanceTime(2 * ONE_DAY_IN_SEC)
      await evm.mine()
    ...
    it('When a .wbt name is in grace period it cannot call setSubnodeOwner', async () => {
      await expect(
        NameWrapper.setSubnodeOwner(
          wrappedTokenId,
          subLabel,
    ...
    it('When a .wbt name is in grace period it cannot call setSubnodeRecord', async () => {
      await expect(
        NameWrapper.setSubnodeRecord(
          wrappedTokenId,
          subLabel,
    ...
    it('When a .wbt name is in grace period it cannot call setRecord', async () => {
      await expect(
        NameWrapper.setRecord(wrappedTokenId, account2, EMPTY_ADDRESS, 0),
      ).to.be.revertedWith(`Unauthorised("${wrappedTokenId}", "${account}")`)
    })

    it('When a .wbt name is in grace period it cannot call safeTransferFrom', async () => {
      await expect(
        NameWrapper.safeTransferFrom(
          account,
          account2,
    ...
    it('When a .wbt name is in grace period it cannot call batchSafeTransferFrom', async () => {
      await expect(
        NameWrapper.safeBatchTransferFrom(
          account,
          account2,
    ...
    it('When a .wbt name is in grace period it cannot call setResolver', async () => {
      await expect(
        NameWrapper.setResolver(wrappedTokenId, EMPTY_ADDRESS),
      ).to.be.revertedWith(`Unauthorised("${wrappedTokenId}", "${account}")`)
    })

    it('When a .wbt name is in grace period it cannot call setTTL', async () => {
      await expect(NameWrapper.setTTL(wrappedTokenId, 0)).to.be.revertedWith(
        `Unauthorised("${wrappedTokenId}", "${account}")`,
      )
    })

    it('When a .wbt name is in grace period it cannot call setFuses', async () => {
      await expect(NameWrapper.setFuses(wrappedTokenId, 0)).to.be.revertedWith(
        `Unauthorised("${wrappedTokenId}", "${account}")`,
      )
    })

    it('When a .wbt name is in grace period it cannot call setChildFuses', async () => {
      await expect(
        NameWrapper.setChildFuses(wrappedTokenId, labelhash('sub'), 0, 0),
      ).to.be.revertedWith(`Unauthorised("${wrappedTokenId}", "${account}")`)
    })

    it('When a .wbt name is in grace period, unexpired subdomains can call setFuses', async () => {
      await NameWrapper2.setFuses(subTokenId, CANNOT_UNWRAP)
      const [, fuses] = await NameWrapper.getData(subTokenId)
      expect(fuses).to.equal(PARENT_CANNOT_CONTROL | CANNOT_UNWRAP)
    })

    it('When a .wbt name is in grace period, unexpired subdomains can transfer', async () => {
    ...
    it('When a .wbt name is in grace period, unexpired subdomains can set resolver', async () => {
      await NameWrapper2.setResolver(subTokenId, account)
      expect(await EnsRegistry.resolver(subTokenId)).to.equal(account)
    })

    it('When a .wbt name is in grace period, unexpired subdomains can set ttl', async () => {
      await NameWrapper2.setTTL(subTokenId, 100)
      expect(await EnsRegistry.ttl(subTokenId)).to.equal(100)
    })

    it('When a .wbt name is in grace period, unexpired subdomains can call setRecord', async () => {
    ...
    it('When a .wbt name is in grace period, unexpired subdomains can call setSubnodeOwner', async () => {
      await NameWrapper2.setSubnodeOwner(subTokenId, 'sub2', account2, 0, 0)
      expect(await NameWrapper.ownerOf(namehash('sub2.sub.test.wbt'))).to.equal(
        account2,
      )
    })

    it('When a .wbt name is in grace period, unexpired subdomains can call setSubnodeRecord', async () => {
      await NameWrapper2.setSubnodeRecord(
        subTokenId,
        'sub2',
        account2,
    ...
    it('When a .wbt name is in grace period, unexpired subdomains can call setChildFuses if the subdomain exists', async () => {
      await NameWrapper2.setSubnodeOwner(subTokenId, 'sub2', account2, 0, 0)
      await NameWrapper2.setChildFuses(subTokenId, labelhash('sub2'), 0, 100)
      const [owner, fuses, expiry] = await NameWrapper.getData(
        namehash('sub2.sub.test.wbt'),
      )
      expect(owner).to.equal(account2)
      expect(expiry).to.equal(100)
      expect(fuses).to.equal(0)
  ...
  describe('Registrar tests', () => {
    ...
    const wrappedTokenId1 = namehash('sub1.wbt')
    ...
    it('Reverts when attempting to call token owner protected function on an unwrapped name', async () => {
      ...
      // XXX: note that at this step, the hackler should use the current .wbt
      // registrar to directly register `sub1.wbt` to himself, without wrapping
      // the name.
      await BaseRegistrar.register(labelHash1, hacker, 10 * ONE_DAY_IN_SEC)
      ...
      //
      // XXX: corrently, `sub1.wbt` becomes a normal node
      await EnsRegistryH.setOwner(wrappedTokenId1, NameWrapper.address)

      // create `sub2.sub1.wbt` to the victim user with `PARENT_CANNOT_CONTROL`
      // burnt.
      await expect(
  ...
  describe('ERC1155 additional tests', () => {
    ...
    it('Approval on the Wrapper does not give permission to wrap the .wbt name', async () => {
      await BaseRegistrar.register(
        labelhash(label),
        account,
        1 * ONE_DAY_IN_SEC,
    ...
    it('Approval on the Wrapper does not give permission to wrap a non .wbt name', async () => {
    ...
    it('When .wbt name expires, it is untransferrable', async () => {
      await BaseRegistrar.register(
        labelhash(label),
        account,
        1 * ONE_DAY_IN_SEC,
  ...
  describe('Implicit Unwrap tests', () => {
    ...
    const wrappedTokenId2 = namehash('sub2.sub1.wbt')
    ...
    it('Trying to burn child fuses when re-registering a name on the old controller reverts', async () => {
      ...
      // create `sub2.sub1.wbt` w/o fuses burnt
      await NameWrapperH.setSubnodeOwner(
        wrappedTokenId1,
        label2,
        hacker,
      ...
      // XXX: PREPARE HACK!
      // set `EnsRegistry.owner` of `sub1.wbt` as the hacker himself.
      ...
      // set `sub2.sub1.wbt` to the victim user w fuses burnt
    ...
    it('Renewing a wrapped, but expired name .wbt in the wrapper, but unexpired on the registrar resyncs expiry', async () => {
      await NameWrapper.registerAndWrapETH2LD(
        label1,
        account,
        1 * ONE_DAY_IN_SEC,
      ...
      await BaseRegistrar.renew(labelHash1, 365 * ONE_DAY_IN_SEC)
